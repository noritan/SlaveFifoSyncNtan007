...
#include "producer.cydsn/cyfxgpif2config.h"
...
CyU3PDmaMultiChannel glChHandleSlFifoPtoU;   /* DMA Multi Channel handle for P2U transfer. */
        ...
        )
{
    ...
    {
    	status =	CyU3PDmaMultiChannelResume(chHandle, CyFalse, CyTrue);
    	if (status != CY_U3P_SUCCESS)
        {
            CyU3PDebugPrint (4, "CyU3PDmaMultiChannelResume failed, Error code = %d\n", status);
        }
    }
}
        ...
        void)
{
    ...
    dmaMCfg.size  = size * burstLen;
    /* Multiply the buffer size with the multiplier
     * for performance improvement. */
    dmaMCfg.count = CY_FX_SLFIFO_DMA_BUF_COUNT;
    dmaMCfg.validSckCount = 2;
    dmaMCfg.prodSckId[0] = CY_FX_PRODUCER_PPORT_SOCKET0;
    dmaMCfg.prodSckId[1] = CY_FX_PRODUCER_PPORT_SOCKET1;
    dmaMCfg.consSckId[0] = CY_FX_CONSUMER_USB_SOCKET;
    dmaMCfg.dmaMode = CY_U3P_DMA_MODE_BYTE;
    /* Enabling the callback for produce event. */
    dmaMCfg.notification = 0;
    dmaMCfg.cb = NULL;
    ...
    apiRetStatus = CyU3PDmaMultiChannelCreate (&glChHandleSlFifoPtoU,
            CY_U3P_DMA_TYPE_AUTO_MANY_TO_ONE, &dmaMCfg);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (4, "CyU3PDmaChannelCreate failed, Error code = %d\n", apiRetStatus);
        CyFxAppErrorHandler(apiRetStatus);
    ...
    apiRetStatus = CyU3PDmaMultiChannelSetXfer (&glChHandleSlFifoPtoU, CY_FX_SLFIFO_DMA_TX_SIZE, 0);
    if (apiRetStatus != CY_U3P_SUCCESS)
    {
        CyU3PDebugPrint (4, "CyU3PDmaChannelSetXfer Failed, Error code = %d\n", apiRetStatus);
        CyFxAppErrorHandler(apiRetStatus);
                ...
                && (wValue == CY_U3P_USBX_FS_EP_HALT))
        {
            if (glIsApplnActive)
            {
                ...
                if (wIndex == CY_FX_EP_CONSUMER)
                {
                    ...
                    CyU3PDmaMultiChannelSetXfer (&glChHandleSlFifoPtoU, CY_FX_SLFIFO_DMA_RX_SIZE, 0);
                    CyU3PDmaMultiChannelSetXfer (&glChHandleSlFifoPtoU, CY_FX_SLFIFO_DMA_RX_SIZE, 1);
                }
